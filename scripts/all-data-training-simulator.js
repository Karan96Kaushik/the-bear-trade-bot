...
const { addMovingAverage, scanZaireStocks, countMATrendRising, 
    ...
    getDateRange} = require("../analytics");
...
let defaultFilePath = `training_1_simulator.csv`
...
async function processStock(stock, date) {
    try {
        ...
        for (let i = 0; i < 2*6*12; i++) { 
            ...
            // df15m = removeIncompleteCandles(df15m, useCached);
            ...
            // console.log(current5mCandle, current15mCandle, current75mCandle)
            
            if (!current5mCandle || !current15mCandle || !current75mCandle) {
                console.log('No data for this interval', intervalTime)
                continue; // Skip if we don't have data for this interval
            }
            
            // Prepare stock data for simulation
            const stockData = {
                sym: stock,
                orderTime: intervalTime,
            ...
            const simulationStart = new Date(intervalTime);
            ...
            simulationResults.push({
                ...
                orderTimestamp: getDateStringIND(simResult.placedAt),
                candleNum: index,
                sym: stock,
                
                // 5m candle data (current)
                high: current5mCandle.high,
                low: current5mCandle.low,
                ...
                // Volume data
                // volume5m: current5mCandle.volume,
...
// Helper function to create a 75-minute candle from 15-minute candles
function get75mCandleForTime(candles15m, time) {
    let df75min = []
    let df15min_copy = [...candles15m]
    ...
    df75min = addMovingAverage(df75min, 'close', 22, 'sma44');
...
defaultFilePath = `training_1_simulator_${interval}_${MA_WINDOW}.csv`
