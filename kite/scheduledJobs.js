...
const { setupBaxterOrders, updateBaxterStopLoss, 
    checkBaxterOrdersStoplossSafety } = require('./baxter');
...
async function setupOrdersFromSheet() {
    try {
        ...
        // Filter out Benoit stocks which are managed by separate job
        stockData = stockData.filter(s => s.source?.toLowerCase() != 'benoit')
...
const scheduleMISJobs = () => {
    ...
    if (ENABLE_BAXTER) {
        ...
        if (ENABLE_BAXTER_ORDERS_SETUP) {
            ...
            // First run at 3:54 AM, then every 3 minutes during trading hours
            // Cancels all pending orders before creating new ones
            const baxterJob = schedule.scheduleJob('25 54,57 3 * * 1-5', baxterJobCB);
            const baxterJob_2 = schedule.scheduleJob('25 */3 4,5,6,7,8 * * 1-5', baxterJobCB);
            const baxterJob_3 = schedule.scheduleJob('25 0-30/3 9 * * 1-5', baxterJobCB);
            sendMessageToChannel('⏰ Baxter Setup Scheduled - ', getDateStringIND(getEarliestTime(baxterJob, baxterJob_2, baxterJob_3)));
        }
        ...
        const ENABLE_BAXTER_STOP_LOSS_SAFETY = true;
        if (ENABLE_BAXTER_STOP_LOSS_SAFETY) {
            const checkBaxterOrdersStoplossSafetyCB = () => {
                sendMessageToChannel('⏰ Check Baxter Orders Safety Monitor Scheduled - ', getDateStringIND(getEarliestTime(checkBaxterOrdersStoplossSafetyJob, checkBaxterOrdersStoplossSafetyJob_2)));
            ...
            const checkBaxterOrdersStoplossSafetyJob = schedule.scheduleJob('40 */30 4,5,6,7,8 * * 1-5', checkBaxterOrdersStoplossSafetyCB);
            const checkBaxterOrdersStoplossSafetyJob_2 = schedule.scheduleJob('40 0,30 9 * * 1-5', checkBaxterOrdersStoplossSafetyCB);
            sendMessageToChannel('⏰ Check Baxter Orders Safety Monitor Scheduled - ', getDateStringIND(getEarliestTime(checkBaxterOrdersStoplossSafetyJob, checkBaxterOrdersStoplossSafetyJob_2)));
        }
    }
